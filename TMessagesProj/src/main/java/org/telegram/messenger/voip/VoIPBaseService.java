package org.zanbilapp.messenger.voip;
...
import org.zanbilapp.messenger.StatsController;
...
import org.ستاره گرام.ui.VoIPActivity;
import org.ستاره گرام.ui.VoIPPermissionActivity;
...
public abstract class VoIPBaseService extends Service implements SensorEventListener, AudioManager.OnAudioFocusChangeListener, VoIPController.ConnectionStateListener{
	...
	public void onCreate() {
		...
		try {
			...
			cpuWakelock = ((PowerManager) getSystemService(POWER_SERVICE)).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "ستاره گرام-voip");
			cpuWakelock.acquire();
	...
	protected void configureDeviceForCall() {
		...
		try{
			if(proximity!=null){
				proximityWakelock=((PowerManager) getSystemService(Context.POWER_SERVICE)).newWakeLock(PROXIMITY_SCREEN_OFF_WAKE_LOCK, "ستاره گرام-voip-prx");
