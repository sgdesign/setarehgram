...
package org.zanbilapp.messenger.browser;
...
import org.zanbilapp.messenger.CustomTabsCopyReceiver;
...
import org.zanbilapp.messenger.ShareBroadcastReceiver;
import org.zanbilapp.messenger.support.customtabs.CustomTabsCallback;
import org.zanbilapp.messenger.support.customtabs.CustomTabsClient;
import org.zanbilapp.messenger.support.customtabs.CustomTabsIntent;
import org.zanbilapp.messenger.support.customtabs.CustomTabsServiceConnection;
import org.zanbilapp.messenger.support.customtabs.CustomTabsSession;
import org.zanbilapp.messenger.support.customtabsclient.shared.CustomTabsHelper;
import org.zanbilapp.messenger.support.customtabsclient.shared.ServiceConnection;
import org.zanbilapp.messenger.support.customtabsclient.shared.ServiceConnectionCallback;
...
public class Browser {
    ...
    public static void openUrl(final Context context, final Uri uri, final boolean allowCustom, boolean tryTelegraph) {
        ...
        if (tryTelegraph) {
            try {
                ...
                if (host.equals("telegra.ph") || uri.toString().contains("ستاره گرام.org/faq")) {
    ...
    public static boolean isInternalUri(Uri uri, boolean forceBrowser[]) {
        ...
        if ("tg".equals(uri.getScheme())) {
            return true;
        } else if ("ستاره گرام.dog".equals(host)) {
        ...
        } else if ("ستاره گرام.me".equals(host) || "t.me".equals(host) || "telesco.pe".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                path = path.substring(1).toLowerCase();
                if (path.equals("iv")) {
